...
import { Category, Post, Subcategory, User } from "../generated/prisma";
import {
  ArrowUpDown,
  ChevronDown,
  ChevronFirst,
...
const BasicTable = ({ data, columns }: any) => {
  ...
  return (
    <div className="overflow-x-auto ">
      <input
        className=" p-1 px-5 m-2  rounded-full focus:outline-gray-700 outline outline-gray-400 placeholder:text-xs"
        placeholder="Search"
        type="text"
        value={filtering}
            ...
            <tr key={headerGroup.id}>
              {headerGroup.headers.map((header) => {
                ...
                return (
                  <th
                    key={header.id}
                    className="px-2 py-3 text-left text-sm   cursor-pointer select-none"
                    ...
                    {isSortable && (
                      <span className="ml-1">
                        {sorted === "asc" ? (
                          ...
                          <ChevronsUpDownIcon className="inline " width={15} />
                        )}
                      </span>
                    )}
                  </th>
